    ...
    <div className="view-section active">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '32px' }}>
        <div className="glass-card" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Total Clients</p>
            <h3 style={{ margin: '4px 0 0', fontSize: '1.5rem', color: 'var(--text-primary)' }}>{totalClients}</h3>
          </div>
        </div>
        <div className="glass-card" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ background: 'rgba(34, 197, 94, 0.2)', padding: '12px', borderRadius: '50%', color: '#16A34A' }}><Activity size={24} /></div>
          <div>
            <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Active Clients</p>
            <h3 style={{ margin: '4px 0 0', fontSize: '1.5rem', color: 'var(--text-primary)' }}>{activeClients}</h3>
          </div>
        </div>
        <div className="glass-card" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '4px 0 0', fontSize: '1.5rem', color: 'var(--text-primary)' }}>{activeProjectsCount}</h3>
          </div>
        </div>
        <div className="glass-card" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ background: '#FFEDD5', padding: '12px', borderRadius: '50%', color: '#EA580C' }}><AlertTriangle size={24} /></div>
          <div>
            <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.875rem' }}>SLA Compliance</p>
            <h3 style={{ margin: '4px 0 0', fontSize: '1.5rem', color: 'var(--text-primary)' }}>{slaCompliance}%</h3>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card" style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
          <h3 style={{ margin: 0, color: 'var(--text-primary)', fontSize: '1.1rem' }}>All Clients</h3>
        ...
        {/* Data Table */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>Loading clients...</div>
        ) : error ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: 'var(--bg-glass)', borderBottom: '1px solid #E2E8F0' }}>
                  <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-secondary)' }}>Client Name</th>
                  <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-secondary)' }}>Primary Contact</th>
                  <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-secondary)' }}>Email</th>
                  <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-secondary)' }}>Phone</th>
                  <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-secondary)', textAlign: 'center' }}>Active Projects</th>
                  <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-secondary)' }}>Status</th>
                  <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-secondary)', textAlign: 'center' }}>SLA Compliance</th>
              ...
              <tbody>
                {filteredClients.map(client => (
                  <tr key={client.id} style={{ borderBottom: '1px solid var(--bg-tertiary)' }}>
                    <td style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span style={{ fontWeight: 500, color: 'var(--text-primary)' }}>{client.name}</span>
                    </td>
                    <td style={{ padding: '16px', color: 'var(--text-secondary)' }}>{client.primary_contact_name || '-'}</td>
                    <td style={{ padding: '16px', color: 'var(--text-secondary)' }}>{client.primary_contact_email || '-'}</td>
                    <td style={{ padding: '16px', color: 'var(--text-secondary)' }}>{client.primary_contact_phone || '-'}</td>
                    <td style={{ padding: '16px', textAlign: 'center', color: 'var(--text-secondary)' }}>{client.activeProjectsCount || 0}</td>
                    <td style={{ padding: '16px' }}>
                      <span style={{ 
                        padding: '4px 10px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 600,
                        background: client.status === 'Active' ? 'rgba(34, 197, 94, 0.2)' : client.status === 'Suspended' ? 'rgba(239, 68, 68, 0.2)' : 'var(--bg-tertiary)',
                        color: client.status === 'Active' ? '#16A34A' : client.status === 'Suspended' ? '#EF4444' : 'var(--text-secondary)'
                      }}>
                        {client.status}
                      </span>
                    </td>
                    <td style={{ padding: '16px', textAlign: 'center', fontWeight: 600, color: 'var(--text-primary)' }}>
                ...
                {filteredClients.length === 0 && (
                  <tr>
                    <td colSpan="8" style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>No clients found matching the criteria.</td>
                  </tr>
                )}
              </tbody>
            </table>

  ...
  return (
    <div style={{ padding: '20px 0', fontFamily: 'Poppins, sans-serif', color: 'var(--text-primary)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: 'Total Learners', value: totalLearners, icon: <BookOpen size={20} color='var(--text-primary)' />, border: 'var(--text-primary)' },
          { label: 'Active Learners', value: activeLearners, icon: <Star size={20} color='var(--primary)' />, border: 'var(--primary)' },
          ...
          { label: 'Pass Percentage', value: `${Math.round(avgQuizScore * 1.05)}%`, icon: <CheckCircle size={20} color='var(--primary)' />, border: 'var(--primary)' },
      ...
      <div style={{ background: 'var(--bg-glass)', borderRadius: '20px', padding: '24px', boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.4), 0 0 20px rgba(0, 240, 255, 0.15), inset 0 0 30px rgba(0, 240, 255, 0.08)', marginBottom: '24px' }}>
        ...
        {activeTab === 'courses' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            {[
              { title: 'Enrollment Rate', val: '96.2%', color: 'var(--text-primary)', detail: '8,420 learners registered' },
              ...
              <div key={i} style={{ padding: '20px', borderRadius: '16px', background: 'var(--bg-tertiary)', border: '1px solid #B7BEC7' }}>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{stat.title}</div>
                <div style={{ fontSize: '1.8rem', fontWeight: 800, marginTop: '8px', color: stat.color }}>{stat.val}</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', marginTop: '6px' }}>{stat.detail}</div>
              </div>
            ))}
          </div>
        )}
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '20px', alignItems: 'center', height: '160px' }}>
                ...
                <svg viewBox="0 0 350 150" style={{ width: '100%', height: '100%' }}>
                  ...
                  <text x="62" y="135" fill='var(--text-secondary)' fontSize="9" fontWeight="600" textAnchor="middle">Q1</text>
                  ...
                  <text x="142" y="135" fill='var(--text-secondary)' fontSize="9" fontWeight="600" textAnchor="middle">Q2</text>

                  {/* Q3 Bar */}
                  <rect x="210" y="30" width="25" height="90" fill='var(--primary)' rx="4" />
                  <text x="222" y="25" fill='var(--primary)' fontSize="9" fontWeight="800" textAnchor="middle">86%</text>
                  <text x="222" y="135" fill='var(--text-secondary)' fontSize="9" fontWeight="600" textAnchor="middle">Q3</text>
                  ...
                  <text x="302" y="135" fill='var(--text-secondary)' fontSize="9" fontWeight="600" textAnchor="middle">Q4</text>
                </svg>
              </div>
            </div>
            <div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead>
                <tr style={{ background: 'var(--bg-tertiary)', borderBottom: '2px solid #B7BEC7' }}>
                  <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 700, color: 'var(--text-secondary)' }}>Trainer Name</th>
                  <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-secondary)' }}>Sessions Conducted</th>
                  <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-secondary)' }}>Feedback Score</th>
                  <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-secondary)' }}>Completion %</th>
                  <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-secondary)' }}>Satisfaction</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={idx} style={{ borderBottom: '1px solid #EEF2F7' }}>
                    ...
                    <td style={{ padding: '10px 12px', textAlign: 'center', color: 'var(--text-secondary)' }}>{row.count} sessions</td>
        ...
        {activeTab === 'skills' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 360px), 1fr))', gap: '24px' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[
                  ...
                  { label: 'Compliance & Safety', score: 94, col: 'var(--text-primary)' }
                ].map((item, idx) => (
                  <div key={idx}>
            ...
            <div>
              ...
              <div style={{ padding: '16px', background: 'var(--bg-tertiary)', borderRadius: '16px', border: '1px solid #B7BEC7', height: '150px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  <button style={{ padding: '6px 12px', background: 'var(--primary)', color: 'var(--bg-glass)', border: 'none', borderRadius: '6px', fontSize: '0.72rem', fontWeight: 700, cursor: 'pointer' }}>Trigger Training</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'live' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 360px), 1fr))', gap: '24px' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[
                  ...
                  <div key={idx} style={{ padding: '14px', border: '1px solid #B7BEC7', borderRadius: '12px', background: 'var(--bg-tertiary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: '0.85rem' }}>{live.title}</div>
                      <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', marginTop: '2px' }}>Host: {live.host} · Room Code: <strong>{live.code}</strong></div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '0.9rem', fontWeight: 800, color: '#EF4444' }}>● {live.active} Active</div>
                      <div style={{ fontSize: '0.68rem', color: 'var(--text-secondary)' }}>Participants</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div style={{ padding: '16px', background: 'var(--text-primary)', borderRadius: '16px', height: '110px', position: 'relative', overflow: 'hidden' }}>
                {/* Floating CSS clouds */}
                <span style={{ position: 'absolute', top: '10px', left: '20px', color: '#38BDF8', fontSize: '1.1rem', fontWeight: 800 }}>Objections</span>
                <span style={{ position: 'absolute', top: '40px', left: '110px', color: 'var(--bg-glass)', fontSize: '1.4rem', fontWeight: 800 }}>Pricing</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 360px), 1fr))', gap: '24px' }}>
        ...
        <div style={{ background: 'var(--bg-glass)', padding: '24px', borderRadius: '20px', boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.4), 0 0 20px rgba(0, 240, 255, 0.15), inset 0 0 30px rgba(0, 240, 255, 0.08)' }}>
          <h3 style={{ margin: '0 0 16px 0', fontSize: '1.1rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Brain size={20} color='var(--primary)' /> AI-Driven L&D Recommendations
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', padding: '12px 16px', background: 'var(--bg-tertiary)', borderRadius: '8px', borderLeft: '4px solid #3E5C8A' }}>
                <span style={{ fontSize: '0.72rem', fontWeight: 800, color: 'var(--primary)', textTransform: 'uppercase' }}>{rec.label}</span>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-primary)' }}>{rec.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-glass)', padding: '24px', borderRadius: '20px', boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.4), 0 0 20px rgba(0, 240, 255, 0.15), inset 0 0 30px rgba(0, 240, 255, 0.08)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {[
              ...
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', border: '1px solid #B7BEC7', borderRadius: '12px', background: 'var(--bg-tertiary)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: cert.border }} />
                  <span style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{cert.label}</span>
                </div>
                <span style={{ fontSize: '0.9rem', fontWeight: 800, color: 'var(--text-primary)' }}>{cert.value}</span>
              </div>
            ))}
          </div>
        </div>

...
async function main() {
  ...
  const htmlPath = path.resolve(__dirname, '..', 'scratch', 'video_storyboard_deck.html');
  ...
  const pdfPath = path.resolve(__dirname, '..', 'frontend', 'public', 'RetailEdge_Video_Presentation.pdf');
  console.log(`Saving PDF slide deck to: ${pdfPath}`);

  await page.pdf({
    path: pdfPath,
    width: '11in',
    height: '6.1875in', // 16:9 landscape aspect ratio
